...
class WebhookController extends AbstractController
{
    ...
    public function webhook(
        ...
        // 1. Vérifier la signature Stripe (Ou l'ignorer localement si la clef n'est pas définie)
        if (!empty($_ENV['STRIPE_WEBHOOK_SECRET']) && $_ENV['STRIPE_WEBHOOK_SECRET'] !== 'whsec_your_webhook_secret_here') {
            try {
            ...
            // Mode de secours local si aucune signature n'est configurée
        ...
        switch ($event->type) {
            ...
            case 'customer.subscription.deleted':
                ...
                if ($userId) {
                    $user = $userRepository->find($userId);
                    $freePlan = $planRepository->findOneBy(['name' => 'Free']) ?? $planRepository->findOneBy(['name' => 'FREE']);
